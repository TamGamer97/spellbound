 ...
 *
 * Usage:
 *   node scripts/v1/generate-puzzles.js [options]
 *   node scripts/v1/generate-puzzles.js --output data/puzzles.json
 *   node scripts/v1/generate-puzzles.js --sql --limit 50
 *
 * Options:
 ...
 *   --limit <n>   Max puzzles to output (default: 1000; uses letter-balanced selection with min % per letter)
...
/** Letters to exclude from generated puzzles (empty = allow all). Set to ['S','Q'] to ban those. */
const BANNED_LETTERS = new Set([]);
...
function generatePuzzles(wordsForLetterSets, wordsForValidWordsSet, minWordsPerPuzzle = MIN_WORDS_PER_PUZZLE, minPangrams = 2) {
  ...
  for (const [key] of letterSetToWords) {
    if (key.length !== 7) continue;
    if (BANNED_LETTERS.size > 0) {
      const hasBanned = [...key].some((c) => BANNED_LETTERS.has(c.toUpperCase()));
      if (hasBanned) continue;
    }
    sevenLetterKeys.push(key);
  }
...
function toSql(puzzles) {
  ...
  return (
    '-- Generated by scripts/v1/generate-puzzles.js\n' +
...
async function main() {
  ...
  const want1000 = (opts.limit == null && 1000 >= 500) || (opts.limit != null && opts.limit >= 500);
  const poolMinWords = want1000 && minWords >= 20 ? 12 : minWords;
  const poolMinPangrams = want1000 && minPangrams >= 2 ? 1 : minPangrams;
  let puzzles = generatePuzzles(wordsDiscovery, validWordsSet, poolMinWords, poolMinPangrams);

  if (opts.minPoints != null && opts.minPoints > 0) {
    puzzles = puzzles.filter((p) => p.total_points >= opts.minPoints);
  }
  const POOL_MIN_POINTS = want1000 ? 0 : 50;
  const POOL_MAX_POINTS = want1000 ? 99999 : 1200;
  ...
  // Generate up to 1000 puzzles with per-letter minimum: every letter used, each at least minPercent of fair share.
  const TARGET_PUZZLE_COUNT = 1000;
  const MIN_LETTER_PERCENT = 0.3;  // each letter in at least 30% of fair share
  ...
  if (puzzles.length < targetCount) {
    process.stderr.write(`Pool has ${puzzles.length} puzzles; outputting all (target was ${opts.limit != null ? opts.limit : TARGET_PUZZLE_COUNT}).\n`);
  }
